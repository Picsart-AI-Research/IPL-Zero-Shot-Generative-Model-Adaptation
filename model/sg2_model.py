...
class Generator(nn.Module):
    def __init__(
        ...
        self.num_layers = (self.log_size - 2) * 2 + 1  # num of layers
        ...
        for i in range(3, self.log_size + 1):
            out_channel = self.channels[2 ** i]  # 8, 16, 32, ....(512, 512, 512, ....)
            # in_channel 512
            self.convs.append(
                StyledConv(
                    in_channel,
    ...
    def forward(
        ...
        if not input_is_s_code:
            return self.forward_with_w(styles, return_latents, inject_index, truncation, truncation_latent,
                                       input_is_latent, noise, randomize_noise)
    ...
    def forward_with_w(
        ...
        if not input_is_latent:  # mapping or not
            styles = [self.style(s) for s in styles]

        if noise is None:  # noise or not
            if randomize_noise:
