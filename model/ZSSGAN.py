...
class SG2Generator(torch.nn.Module):
    ...
    def forward(self,
        ...
        if truncation_latent is None:
            return self.generator(styles, return_latents=return_latents, truncation=truncation, truncation_latent=self.mean_latent,
                                  noise=noise, randomize_noise=randomize_noise, input_is_latent=input_is_latent, input_is_s_code=input_is_s_code)
        else:
            return self.generator(styles, return_latents=return_latents, truncation=truncation,
                                  truncation_latent=truncation_latent,
                                  noise=noise, randomize_noise=randomize_noise, input_is_latent=input_is_latent,
                                  input_is_s_code=input_is_s_code)

class SG2Discriminator(torch.nn.Module):
    def __init__(self, checkpoint_path, img_size=256, channel_multiplier=2, device="cuda"):
...
class ZSSGAN(torch.nn.Module):
    def __init__(self, args):
        ...
        self.generator_trainable = SG2Generator(args.train_gen_ckpt,
                                                img_size=args.size,
                                                channel_multiplier=args.channel_multiplier,
                                                is_train=True,
                                                batch=self.args.batch).to(self.device)
    ...
    def get_clip_loss_models(self):
        return self.clip_loss_models, self.clip_model_weights
    ...
    def determine_opt_layers(self):
        ...
        layer_weights = torch.abs(w_codes - initial_w_codes).mean(dim=-1).mean(dim=0)  # variation of latent code
        chosen_layer_idx = torch.topk(layer_weights, self.auto_layer_k)[1].cpu().numpy()  # top k
        ...
        chosen_layers = [idx_to_layer[idx] for idx in chosen_layer_idx]  # select layers
    ...
    def forward(
        ...
        with torch.no_grad():
            if input_is_latent:
                ...
                w_styles = self.generator_frozen.style(styles)  # mapping
